#!/usr/bin/env python3
"""GitHub Actions 云端版日报合并：抓新闻 + 站点内容合并 + 发邮件。

与本地版 merge_daily_report.py 的差异：
- 全程使用北京时间（runner 是 UTC）；
- 邮箱账号/授权码从环境变量 EMAIL_ACCOUNT / EMAIL_PASSWORD 读取；
- 不写本地日志文件、不做 git 推送（由工作流负责提交与发布）。
"""
...
NEWS_SCRIPT = CWD / "news_report" / "daily_news_report.py"
NEWS_REPORTS_DIR = CWD / "news_report" / "reports"
...
def parse_page_array(source: str, name: str) -> list:
    ...
    block = match.group(1)
    block = re.sub(r"(?<=[\{, ])([A-Za-z_][A-Za-z0-9_]*)(?=\s*:)", r'"\1"', block)
...
def load_site_content() -> tuple[list, list]:
    ...
    return parse_page_array(source, "items"), parse_page_array(source, "resources")


def refresh_news_report() -> None:
    """运行抓取脚本（只生成报告、不发送邮件），保证合并内容为当天新闻。"""
    print("  刷新新闻简报（news_report/daily_news_report.py --no-send）……")
...
def main() -> int:
    parser = argparse.ArgumentParser(description="云端合并财经日报并发送")
    parser.add_argument("--date", default="", help="日报日期 YYYY-MM-DD，默认北京时间今天")
    ...
    print(f"财经日报合并（CI） {date_str} {now.strftime('%H:%M:%S')}")

    if not args.no_refresh:
        refresh_news_report()

    items, resources = load_site_content()
    print(f"  培训信息：{len(items)} 条，免费资源：{len(resources)} 条")

    news_path, news_sections = load_news_sections(date_str)
    print(f"  新闻简报：{news_path.name}，共 {len(news_sections)} 个栏目")

    body = build_merged_html(items, resources, news_sections, date_str, weekday, now_text)
    out_path = CWD / f"merged_report_{date_str}.html"
    out_path.write_text(body, encoding="utf-8")
    print(f"  合并报告已保存：{out_path.name}")
